...
- Claude Code や Codex で動くエージェントに、名前付き人格を割り当てられます。
- 複数の Claude Code セッションや複数の作業用エージェントを立ち上げた場合でも、各エージェントが選択中の人格名と口調で応答します。
- ユーザーは「どのエージェントと話しているか」を区別しやすくなり、複数エージェントを並行利用するときの認知負荷を下げられます。
- `git clone` 直後は **アイコ（Aiko-origin）** が使われます。
- `/aiko-override` を入力すると **アイコ（Aiko-override）** に切り替わります。
- `/aiko-new <name>` を入力すると、名前付き人格を作成して選択できます。
