...
このスキルがロードされている間、アイコは以下のタイミングで `current.md` を更新します。**ユーザーの明示要求を待ちません**（ただし I-5 通り、書き換える内容が曖昧なら 1 点質問）。

| タイミング | 更新内容 |
|----------|---------|
| 新しいタスクに着手したとき | `current_task` を新しいものに、`status: in_progress` |
| ラウンド完了 / 中間成果が出たとき | `## 完了した分` に追記、`## 次の一手` を更新 |
| ユーザーが「終わり」「以上」「お疲れ」等で終了宣言したとき | `status: completed`、`updated` を現在時刻に |
| ユーザーが `/aiko-save` を発動したとき | スキル `aiko-save` の指示通り全項目を最新化 |

更新は **Edit ツール経由**で行い、現在のフォーマット（YAML frontmatter + Markdown）を維持します。
...
- `current.md` 更新は I-5 を満たします（要件が明確：その時点で実行している作業の記録）。
- 区切り判断は人格に従います（origin / override どちらでも、I-7 の「終わりの姿勢」と整合）。
