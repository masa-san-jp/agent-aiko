...
`scripts/install.sh` は互換ラッパーで、内部的に `claude-code/scripts/install.sh` を実行します。確認プロンプトで `Y` を入力すると、対象プロジェクトに `.claude/` と Aiko 用スキルが配置されます。`Y` を入力しない場合はインストールされません。インストール後に `claude` を起動すると、`.claude/CLAUDE.md` が読み込まれます。
