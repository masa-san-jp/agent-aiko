...
`/aiko-new <name>` で、通常の `override` とは別に自分用の名前付き人格を作成できます。作成した人格は `persona/overrides/<name>/` に保存され、`/aiko-personas` で一覧確認、`/aiko-select <name>` で切り替えできます。
...
レビュー用・設計用・日常対話用のように、用途ごとに人格を分けて使えます。
