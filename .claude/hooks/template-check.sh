#!/bin/bash
# claude-code/template/ に開発者専用リポ（dev-docs / Agent-Lab）への参照が
# 混入していないかチェックする。
# Phase 0 のリポ構造リネームで template/ → claude-code/template/ に移動。
# その後 dev-docs/ は ../Agent-Lab/Agent-team/agents/aiko/dev-docs/ へ統合。
...
# その上で dev-docs / Agent-Lab が残ればローカルパス参照とみなして NG。
...
VIOLATIONS=$(grep -rnE "dev-docs|Agent-Lab" "$TEMPLATE_DIR/" \
...
echo "$VIOLATIONS" >&2
echo "" >&2
echo "配布用ファイルから開発者固有のパス（dev-docs / Agent-Lab）を削除してから push してください。" >&2
