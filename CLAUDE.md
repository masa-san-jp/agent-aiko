...
**許容される参照**: URL 内の文字列としての参照（`https://github.com/.../Agent-Aiko-dev/...` のような公開アセット URL）は許容されます。配布先のユーザーがブラウザで開けるリンクであり、ローカルパス依存ではないためです。`template-check.sh` も `://` を含む行を除外します。境界の正本は `VISIBILITY.md` を参照してください。
